...
function fetchChannel() {
  channelUrl = `${slackApiUrl}/channels.info?token=${token}&channel=${channelId}&pretty=1`;
  const res = request('GET', channelUrl);
...
function fetchMembers(ids) {
  let members = [];
  ids.map((id) => {
    channelUrl = `${slackApiUrl}/users.info?token=${token}&user=${id}&pretty=1`
    const res = request('GET', channelUrl);
